<?php
  ...
  if (!isset($_SERVER['PHP_AUTH_USER']) || !isset($_SERVER['PHP_AUTH_PW'])) {
    // The username and pwd weren't entered so send the authentication headers
  ...
  if (mysqli_num_rows($data) == 1) {
    // The log-in is confirmed, so set the user ID and username variables
    $row = mysqli_fetch_array($data);
    $user_id = $row['user_id'];
    $username = $row['username'];
  }
  else {
    // The credentials are incorrect, so resend the authentication headers
    header('HTTP/1.1 401 Unauthorized');
    header('WWW-Authenticate: Basic realm="Mismatch"');
